...
class _DA_CALL_CONVERT_NO_ARGS_FOUND(Exception):
...
def _da_call_convert(insts: dal.InstGrp,
                     o: typ.Any,
                     mode: int = ARG_EMPTY) -> typ.Any:
    ...
    if mode & ARG_EMPTY:
        ...
        if args is None:
            raise _DA_CALL_CONVERT_NO_ARGS_FOUND()
    ...
    if mode & ARG_PROVIDE:
        # This Call expression is argument

    return o


def _da_call_no_arguments(insts: dal.InstGrp, o: core.DType) -> typ.Any:
    op = o.compileInfo
    assert isinstance(op, core.Operation)
...
def _da_call_with_arguments(insts: dal.InstGrp, o: core.DType) -> typ.Any:
    op = o.compileInfo
    assert isinstance(op, core.Operation)

    # Arguments is provided by _da_call_provide_arguments evaluations
    args = insts.getFlag(flags.CALL_ARGUMENTS)

    assert isinstance(args, core.DList)
    for arg in args:
        assert isinstance(arg, core.DStr)
...
def _da_call_provide_arguments(insts: dal.InstGrp, o: core.DType) -> typ.Any:
    op = o.compileInfo
    assert isinstance(op, core.Operation)
